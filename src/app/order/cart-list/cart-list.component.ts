...
export class CartListComponent implements OnInit, OnDestroy {
  ...
  activeSearchMode(mode: string): void {
    if (mode === SearchMode.ACCOUNT || (mode === SearchMode.PRODUCT && this.accountInfo)) {
      ...
      setTimeout(() => { this.searchText.nativeElement.value = ''; this.searchText.nativeElement.focus(); }, 1520);
    }
  }
  ...
  private stockMessage(product: Product, message: string) {
    ...
    try {
      if (product.images === null) {
        ...
        if ((product.images[1].url).indexOf('http') > -1) {
        ...
        this.cartList[index].entryNumber).subscribe(
          result => {
            ...
            this.setPage(this.cartList.length % this.cartListCount === 0 ? Math.ceil((this.cartList.length) / this.cartListCount) :
                                                                           Math.ceil((index + 1) / this.cartListCount));
            if (this.orderType === OrderType.GROUP) {
              // 그룹 카트 조회
              this.getGroupCart(this.cartInfo.user.uid, this.cartInfo.code);
            }
 ...
 */
  ...
  totalPriceInfo(): void {
    ...
    this.discount = this.resCartInfo.cartList && this.resCartInfo.cartList.totalDiscountWithTax ? this.resCartInfo.cartList.totalDiscountWithTax.value : 0;
  ...
  doPageUp(evt: any) {
    if (this.cartList.length > this.cartListCount) {
      this.setPage(this.pager.currentPage - 1, true);
    } else {
      if (this.orderType === OrderType.GROUP) {
