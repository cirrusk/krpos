...
import { Product, ProductData } from '../../data/model';
...
export class ProductDataProvider {
  ...
  public searchProductCode(code: string): null | ProductData {
    for (const item of this.products) {
        if (item.code === code) {
            console.log(`Found name ${item.name}`);
            return new ProductData(item.code, item.name, item.price, item.qty, item.barcode);
        }
    }
  ...
  public searchBarcode(barcode: string): null | ProductData {
    for (const item of this.products) {
