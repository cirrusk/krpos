 ...
 */
  private createCustomerAccount(sponsorNumber: string) {
    ...
    this.createAccountSubscription = this.accountService.createNewAccount(registerType, this.phonetype, this.userPhone, sponsorNumber).subscribe(userInfo => {
      if (userInfo) {
        if (userInfo.message) {
          this.errorMessage = userInfo.message ? userInfo.message.returnMessage : '오류가 발생하였습니다. 관리자에게 문의하시기 바랍니다.';
          return;
        }
      ...
      if (errdata) {
        ...
        if (errdata.type === 'InvalidDmsError') {
          ...
          this.errorMessage = `'${this.userPhone.replace(/(^02.{0}|^01.{1}|[0-9]{3})([0-9]+)([0-9]{4})/, "$1-$2-$3")}' 은 이미 등록된 번호입니다.`;
        } else {
          this.errorMessage = errdata.message ? errdata.message : '오류가 발생하였습니다. 관리자에게 문의하시기 바랍니다.';
        }
      }
